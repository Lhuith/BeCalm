        ...
        void ExecuteThread()
        {
            ...
            while (!m_ShouldStopThreads && m_ActionQueue != null)
            {
                ...
                catch (Exception e)
                {
                    ...
                    {
                        FluvioBase.LogError(string.Format("Unhandled {0} caught on background thread ({1}) - task aborted.\n{2}", e.GetType().Name, e.Message, e.StackTrace), "Fluvio", "Parallel.ExecuteThread");
                    }
                }
            }
        }
